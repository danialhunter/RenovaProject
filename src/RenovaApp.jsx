 ...
 * Clean JS/JSX version (no TypeScript syntax)
...
const INITIAL_INVENTORY = [
  {
    ...
    available: true,
    location: "Bin A1",
    image: "https://images.unsplash.com/photo-1589792923962-537704632910?auto=format&fit=crop&w=300&q=80",
    description: "Assorted sizes of cardboard tubes."
  },
  {
    id: 2,
    name: "Fabric Scraps",
    ...
    available: true,
    location: "Shelf B2",
    image: "https://images.unsplash.com/photo-1521980066195-207d58a5e12f?auto=format&fit=crop&w=300&q=80",
    description: "Colorful cotton offcuts."
  },
  {
    id: 3,
    name: "Wine Corks",
    ...
    available: true,
    location: "Bin A3",
    image: "https://images.unsplash.com/photo-1516546700755-e87f2258544d?auto=format&fit=crop&w=300&q=80",
    description: "Natural corks."
  }
];
...
const Button = ({
  ...
  const baseStyle =
    "px-5 py-2.5 rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2 focus:ring-2 focus:ring-offset-2 tracking-wide disabled:opacity-50 disabled:cursor-not-allowed";

  const variants = {
    primary: "bg-[#e93949] hover:bg-[#d63443] text-white focus:ring-[#e93949] shadow-sm hover:shadow-md",
    secondary:
      "bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-100 focus:ring-slate-500",
    danger: "bg-red-500 hover:bg-red-600 text-white focus:ring-red-500 shadow-sm",
    outline:
      "border-2 border-slate-200 dark:border-slate-600 hover:border-[#e93949] text-slate-600 dark:text-slate-300 hover:text-[#e93949]",
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-3 pt-4">
        ...
        <Button type="submit" className="flex-1">
          {mode === "edit" ? t("saveChanges") : t("addItem")}
        </Button>
      </div>
    </form>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex gap-3 pt-2">
        ...
        <Button type="submit" variant="primary" className="flex-1 bg-emerald-600 hover:bg-emerald-700">
          {t("confirmReturn")}
        </Button>
      </div>
    </form>
...
// --- MAIN INNER APP (logic + views) ---

function RenovaAppInner() {
  const [lang, setLang] = useState("en");
  const [viewMode, setViewMode] = useState("landing"); // 'landing' | 'public' | 'admin'
  const [currentUser, setCurrentUser] = useState(null); // user object or null
  const [activeTab, setActiveTab] = useState("inventory"); // 'inventory' | 'return' | 'activity' | 'admin'
  ...
  const [landingBgColor, setLandingBgColor] = useState(
    () => localStorage.getItem("renova_landing_bg_color") || ""
  );
  const [landingBgImage, setLandingBgImage] = useState(
    () => localStorage.getItem("renova_landing_bg_image") || ""
  );

  // Persistence
  useEffect(() => {
    localStorage.setItem("renova_inventory", JSON.stringify(inventory));
  }, [inventory]);
  ...
  useEffect(() => {
    localStorage.setItem("renova_logs", JSON.stringify(logs));
  }, [logs]);

  useEffect(() => {
    localStorage.setItem("renova_loans", JSON.stringify(activeLoans));
  }, [activeLoans]);

  useEffect(() => {
    localStorage.setItem("renova_org_name", orgName);
  }, [orgName]);

  useEffect(() => {
    localStorage.setItem("renova_app_logo", appLogo);
  }, [appLogo]);

  useEffect(() => {
    localStorage.setItem("renova_landing_bg_color", landingBgColor);
  }, [landingBgColor]);

  useEffect(() => {
    localStorage.setItem("renova_landing_bg_image", landingBgImage);
  }, [landingBgImage]);

  // GLOBAL STYLE (remove margin)
  useEffect(() => {
  ...
  const logActivity = (action, itemName, className, note, imageUrl) => {
    const newLog = {
      id: Date.now(),
  ...
  const handleLogoUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        addNotification("success", "Logo updated");
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleBgImageUpload = (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setLandingBgImage(reader.result);
        addNotification("success", "Background updated");
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const LandingView = () => {
    ...
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center p-6 font-sans relative ${defaultClasses}`}
        ...
        <div className="relative z-10 w-full max-w-4xl flex flex-col items-center">
            ...
            <div className="text-center mb-12 animate-in fade-in zoom-in duration-500 max-w-2xl mx-auto w-full">
              ...
              <h1
                className={`text-4xl md:text-5xl font-bold mb-4 tracking-tight leading-tight ${
                  landingBgImage ? "text-white" : "text-slate-900 dark:text-white"
                }`}
              >
                {t("appTitle")}
              </h1>
              <p
                className={`text-lg md:text-xl font-light ${
                  landingBgImage ? "text-white/90" : "text-slate-500 dark:text-slate-400"
                }`}
              >
                {t("appSubtitle")}
              </p>
          ...
          {step === "login" && (
            <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 animate-in fade-in duration-300 relative border border-slate-100 dark:border-slate-700">
              ...
              {!selectedAdminUser ? (
                <div className="space-y-4">
                  <p className="text-center text-slate-500 mb-4 font-medium">{t("selectUser")}</p>
                  {users && users.length > 0 ? (
                    ...
                    <div className="text-center text-red-500 p-4">
                      No users found. Please restart app to reset.
                    </div>
                  )}
                </div>
              ) : (
                <div className="space-y-6">
                  ...
                  <Button className="w-full py-3 text-lg" onClick={verifyPassword}>
                    {t("login")}
                  </Button>
                  <button
                    onClick={() => {
    ...
    return (
      <div className="space-y-6 md:space-y-8 font-sans pb-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
          ...
          {viewMode === "admin" && (
            <Button onClick={() => setAddItemModalOpen(true)} icon={Plus}>
              {t("addItem")}
            </Button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {filteredItems.map((item) => (
            <Card
              key={item.id}
              className={`flex flex-col h-full group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ${
            ...
            >
                  ...
                  <div className="absolute top-4 right-4 flex gap-2">
                    <button
                      ...
                      className={`p-2 rounded-full text-white transition-all shadow-md ${
                        item.available ? "bg-slate-600 hover:bg-slate-800" : "bg-emerald-600 hover:bg-emerald-700"
                      }`}
              ...
              <div className="p-5 md:p-6 flex-1 flex flex-col">
                <div className="text-xs font-bold text-[#ecb282] uppercase tracking-widest mb-2">
                  {item.category}
                </div>
                <h3 className="text-lg md:text-xl font-bold text-slate-800 dark:text-white mb-2 leading-tight">
                  {item.name}
                </h3>
                <p className="text-slate-600 dark:text-slate-400 text-sm mb-6 flex-1 line-clamp-3 leading-relaxed break-words">
                  {item.description}
                </p>
  ...
  const ReturnView = () => {
    ...
    const classLoans = safeActiveLoans.filter((l) => {
      ...
      if (!l.className) return false;
      return l.className.trim().toLowerCase().includes(searchClass.trim().toLowerCase());
    });
          ...
          {classLoans.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {classLoans.map((loan) => (
                <Card
                  key={loan.id}
                  className="flex flex-col hover:shadow-lg transition-shadow border-l-4 border-l-[#e93949]"
                >
                  <div className="p-6 flex-1">
                    ...
                    <div className="space-y-4 text-sm text-slate-600 dark:text-slate-400">
                      <div className="flex items-center gap-2">
                        <UserCircle size={16} />{" "}
                        <span className="font-semibold text-base">{loan.className}</span>
                      </div>
                ...
                {safeLogs && safeLogs.length > 0 ? (
                  safeLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-colors">
                      <td className="p-5 whitespace-nowrap font-medium align-top">
                        {new Date(log.date).toLocaleDateString()}
                      </td>
                      <td className="p-5 align-top">
                        <div className="font-bold text-slate-900 dark:text-white break-words max-w-[150px]">
                          {log.userName}
                        </div>
            ...
            <Card className="p-8 h-fit">
              ...
              <div className="space-y-6">
                ...
                <div className="pt-6 border-t border-slate-200 dark:border-slate-700">
                  <h4 className="text-sm font-bold text-red-500 mb-4 uppercase tracking-wider">
                    {t("dangerZone")}
                  </h4>
                  <Button variant="danger" onClick={clearData} icon={RotateCcw}>
                    {t("resetSystem")}
                  </Button>
                </div>
              </div>
            ...
            <Card className="p-8 h-fit">
              ...
              {currentUser && currentUser.role === "admin" && (
                <div className="mb-8 p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 space-y-3">
                  ...
                  <Button onClick={handleAddUser} className="w-full" icon={Plus}>
                    {t("addUser")}
                  </Button>
                </div>
              )}
              ...
              <div className="space-y-3">
                {safeUsers.map((u) => (
                  <div
                    key={u.id}
                    className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700"
                  >
                    <div className="flex items-center gap-3">
  ...
  const renderContent = () => {
    ...
    if (activeTab === "activity") return <ActivityLogView />;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-3 py-1.5 rounded-full text-xs font-semibold border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-[#e93949] hover:text-[#e93949]"
            >
              {lang === "en" ? "EN / 中文" : "中文 / EN"}
            </button>
      ...
      <nav className="border-b border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 backdrop-blur">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex gap-2 py-2 overflow-x-auto">
          <button
          ...
          >
            <RotateCcw size={16} /> {t("returnItem")}
          </button>
          ...
          {isAdminMode && (
            ...
            >
              <Settings size={16} /> {t("adminSettings")}
            </button>
          )}
        </div>
      </nav>
      ...
      <main className="flex-1">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">{renderContent()}</div>
      </main>
      ...
      <div className="fixed bottom-4 right-4 space-y-3 z-[9999]">
        {notifications.map((n) => (
          <div
            key={n.id}
            className={`px-4 py-3 rounded-xl shadow-lg text-sm flex items-center gap-3 max-w-xs ${
              n.type === "success"
