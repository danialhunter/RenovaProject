...
import { 
  ...
  UserCircle, Key, Image as ImageIcon, Upload, X, Camera, FileSpreadsheet,
  Eye, EyeOff, Lock, Palette, RefreshCcw, Pencil
} from 'lucide-react';
...
const AutoScaleText = ({ text, className = "" }) => {
  const str = String(text || "0"); // Default to "0" if null/undefined
...
const Modal = ({ children, onClose, title }) => (
  // Updated Modal z-index and positioning to ensure it sits on top of everything
  <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300 font-sans" style={{ top: 0, left: 0, right: 0, bottom: 0 }}>
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-xl flex gap-4 items-center border border-slate-100 dark:border-slate-600">
        {item.image ? (
            ...
            <div className="w-16 h-16 bg-slate-200 dark:bg-slate-600 rounded-lg flex items-center justify-center"><Package size={24}/></div>
        )}
        <div>
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800/30 text-emerald-800 dark:text-emerald-200">
            <div className="text-sm font-bold opacity-70 uppercase tracking-wide">Returning</div>
...
export default function RenovaApp() {
  ...
  const [users, setUsers] = useState(() => {
    try {
      const saved = localStorage.getItem('renova_users');
      const parsed = saved ? JSON.parse(saved) : null;
  ...
  const [logs, setLogs] = useState(() => {
    try {
      const saved = localStorage.getItem('renova_logs');
      const parsed = saved ? JSON.parse(saved) : null;
  ...
  const [activeLoans, setActiveLoans] = useState(() => {
    try {
      const saved = localStorage.getItem('renova_loans');
      const parsed = saved ? JSON.parse(saved) : null;
      return Array.isArray(parsed) ? parsed : [];
    } catch { return []; }
  });
  ...
  // Config States
  const [orgName, setOrgName] = useState(() => localStorage.getItem('renova_org_name') || "");
  const [appLogo, setAppLogo] = useState(() => localStorage.getItem('renova_app_logo') || "");
  const [landingBgColor, setLandingBgColor] = useState(() => localStorage.getItem('renova_landing_bg_color') || "");
  const [landingBgImage, setLandingBgImage] = useState(() => localStorage.getItem('renova_landing_bg_image') || "");

  // Persistence
  useEffect(() => localStorage.setItem('renova_inventory', JSON.stringify(inventory)), [inventory]);
  useEffect(() => localStorage.setItem('renova_users', JSON.stringify(users)), [users]);
  useEffect(() => localStorage.setItem('renova_logs', JSON.stringify(logs)), [logs]);
  useEffect(() => localStorage.setItem('renova_loans', JSON.stringify(activeLoans)), [activeLoans]);
  useEffect(() => localStorage.setItem('renova_org_name', orgName), [orgName]);
  useEffect(() => localStorage.setItem('renova_app_logo', appLogo), [appLogo]);
  useEffect(() => localStorage.setItem('renova_landing_bg_color', landingBgColor), [landingBgColor]);
  useEffect(() => localStorage.setItem('renova_landing_bg_image', landingBgImage), [landingBgImage]);
  
  // GLOBAL STYLE INJECTION TO FIX WHITE BARS
  useEffect(() => {
    // This removes the default browser margins and forces the app to fill the screen
  ...
  const handleToggleAvailability = (itemId) => {
    setInventory(prev => prev.map(item => item.id === itemId ? { ...item, available: !item.available } : item));
  };
  ...
  const handleLogoUpload = (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => { setAppLogo(reader.result); addNotification('success', 'Logo updated'); };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleBgImageUpload = (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => { setLandingBgImage(reader.result); addNotification('success', 'Background updated'); };
      reader.readAsDataURL(file);
    }
  };
  ...
  const exportLogsToCSV = () => {
    ...
    const csvContent = [headers.join(","), ...logs.map(log => [new Date(log.date).toLocaleString(), `"${log.userName}"`, log.userRole, log.action, `"${log.itemName}"`, `"${log.note || ''}"`, log.imageUrl || ''].join(","))].join("\n");
  ...
  const BrandLogo = ({ size = "normal" }) => {
     ...
     return (<div className={`${containerClass} bg-[#e93949] text-white flex items-center justify-center shadow-sm shrink-0`}><Leaf size={iconSize} /></div>);
  };

  const ChangePasswordModal = ({ onClose }) => {
    const [pass, setPass] = useState('');
    return (<Modal onClose={onClose} title={t('changePassword')}><div className="space-y-4"><Input type="password" label={t('newPassword')} value={pass} onChange={e => setPass(e.target.value)} required /><Button onClick={() => { handleChangePassword(pass); onClose(); }} className="w-full">{t('changePassword')}</Button></div></Modal>);
  };
  ...
  const LandingView = () => {
    ...
    return (
        <div className={`min-h-screen flex flex-col items-center justify-center p-6 font-sans ${defaultClasses}`} style={containerStyle}>
           ...
           <div className="relative z-10 w-full max-w-4xl flex flex-col items-center">
               ...
               {step !== 'login' && (
                   <div className="text-center mb-12 animate-in fade-in zoom-in duration-500 max-w-2xl mx-auto w-full">
                      <div className="flex justify-center mb-8 h-24 items-center">
                         <img src={logoSource} alt="Logo" className="max-h-32 max-w-[240px] object-contain drop-shadow-xl" onError={(e) => { e.target.onerror = null; e.target.style.display='none'; document.getElementById('fallback-logo').style.display='flex'; }} />
                         <div className="hidden" id="fallback-logo"><BrandLogo size="large" /></div>
                      </div>
                      <h1 className={`text-4xl md:text-5xl font-bold mb-4 tracking-tight leading-tight ${landingBgImage ? 'text-white' : 'text-slate-900 dark:text-white'}`}>{t('appTitle')}</h1>
                      <p className={`text-lg md:text-xl font-light ${landingBgImage ? 'text-white/90' : 'text-slate-500 dark:text-slate-400'}`}>{t('appSubtitle')}</p>
                   </div>
               )}

               {step === 'lang' && (
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-xl">
                      <button onClick={() => { setLang('en'); setStep('selection'); }} className="p-6 bg-white/90 dark:bg-slate-800/90 backdrop-blur rounded-2xl shadow-xl border-2 border-transparent hover:border-[#e93949] transition-all text-center hover:scale-[1.02] duration-300">
                         <div className="text-xl font-bold text-slate-800 dark:text-white">English</div>
                      </button>
                      <button onClick={() => { setLang('zh'); setStep('selection'); }} className="p-6 bg-white/90 dark:bg-slate-800/90 backdrop-blur rounded-2xl shadow-xl border-2 border-transparent hover:border-[#e93949] transition-all text-center hover:scale-[1.02] duration-300">
               ...
               {step === 'selection' && (
                   <>
                   <button onClick={() => setStep('lang')} className={`absolute top-0 left-0 flex items-center gap-2 px-4 py-2 rounded-full ${landingBgImage ? 'text-white hover:bg-white/20' : 'text-slate-500 hover:text-[#e93949] hover:bg-slate-100'}`}>
                       <Globe size={18} /> {lang === 'en' ? 'Language' : '语言'}
                   </button>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full animate-in slide-in-from-bottom-8 duration-500">
                       <button onClick={handlePublicEntry} className="bg-[#e93949] text-white rounded-3xl p-10 shadow-2xl hover:bg-[#d63443] transition-all text-left flex flex-col justify-between h-80 hover:-translate-y-2 group">
                           <div>
                           <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center mb-6 backdrop-blur-sm group-hover:scale-110 transition-transform"><UserCircle size={36} /></div>
                           ...
                           <div className="flex items-center gap-2 font-bold text-lg mt-4">Go <ChevronRight size={24} /></div>
                       </button>
                       <button onClick={() => setStep('login')} className="bg-white/95 dark:bg-slate-800/95 backdrop-blur text-slate-800 dark:text-white rounded-3xl p-10 shadow-xl border border-slate-200 dark:border-slate-700 hover:border-[#e93949] transition-all text-left flex flex-col justify-between h-80 hover:-translate-y-2 group">
                           <div>
                           <div className="w-16 h-16 bg-slate-100 dark:bg-slate-700 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform"><Settings size={36} className="text-slate-600 dark:text-slate-300" /></div>
                           ...
                           <div className="flex items-center gap-2 font-bold text-lg mt-4 text-slate-600 dark:text-slate-300 group-hover:text-[#e93949]">Login <ChevronRight size={24} /></div>
                       </button>
                   </div>
                   </>
               )}
               ...
               {step === 'login' && (
                   <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 animate-in fade-in duration-300 relative border border-slate-100 dark:border-slate-700">
                       <button onClick={() => { setStep('selection'); setSelectedAdminUser(null); setPasswordInput(''); }} className="absolute top-6 left-6 text-slate-500 hover:text-[#e93949] p-2"><X size={24} /></button>
                       ...
                       {!selectedAdminUser ? (
                           <div className="space-y-4">
                               ...
                               {users.length > 0 ? users.map(u => (
                                   <button key={u.id} onClick={() => setSelectedAdminUser(u)} className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 hover:bg-[#e93949]/10 hover:text-[#e93949] dark:hover:text-[#e93949] text-slate-600 dark:text-slate-300 transition-all flex items-center gap-3 border border-transparent hover:border-[#e93949]/30">
                                   <Key size={20} className="text-slate-400" />
                                   <div className="text-left"><div className="font-bold">{u.name}</div><div className="text-xs uppercase tracking-wider opacity-70">{t(u.role)}</div></div>
                                   </button>
                               )) : (
                           ...
                           <div className="space-y-6">
                               ...
                               <div className="space-y-2">
                                   ...
                                   <Input type="password" value={passwordInput} onChange={e => setPasswordInput(e.target.value)} className="text-center text-lg h-12" />
                               </div>
                               <Button className="w-full py-3 text-lg" onClick={verifyPassword}>{t('login')}</Button>
                               <button onClick={() => { setSelectedAdminUser(null); setPasswordInput(''); }} className="w-full text-center text-sm text-slate-400 hover:text-slate-600 underline">{t('cancel')}</button>
                           </div>
                       )}
                   </div>
               )}
  ...
  const InventoryView = () => {
    ...
    // Safeguard for inventory not being an array (though initialization fixes this, runtime mutations could still happen)
            ...
            <Card key={item.id} className={`flex flex-col h-full group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ${!item.available ? 'grayscale opacity-70' : ''}`}>
              ...
              <div className="p-5 md:p-6 flex-1 flex flex-col">
                ...
                <div className="flex items-center justify-between mt-auto pt-5 border-t border-slate-100 dark:border-slate-700">
                  <div className="text-xs font-medium text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded truncate max-w-[120px]">{t('loc')}: {item.location}</div>
                  {viewMode === 'public' && (
                    <Button 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-600 dark:text-slate-400">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-900/50">
                {safeLogs && safeLogs.length > 0 ? safeLogs.map(log => (
                  ...
                  <tr><td colSpan="7" className="p-8 text-center">No activity found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
                ...
                <div>
                    ...
                    <div className="p-4 bg-slate-100 dark:bg-slate-700 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-[#e93949] transition-colors relative">
                      ...
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <button onClick={(e) => { e.preventDefault(); setLandingBgImage(""); }} className="text-xs text-red-500 z-10 underline">Clear Image</button>
                      </div>
                    </div>
                </div>
            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="mt-2 text-center">
                           <button onClick={handleResetLogo} className="text-xs text-red-500 underline hover:text-red-700 flex items-center justify-center gap-1 mx-auto">
                ...
                <div className="space-y-3">
                    {safeUsers.map(u => (
                        <div key={u.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-[#ecb282] flex items-center justify-center text-white font-bold text-xs">{u.name.charAt(0)}</div>
                                <div>
  ...
  const NavItem = ({ id, icon: Icon, label }) => (
    <button
      ...
      className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-xl transition-all duration-200 font-medium ${
        activeTab === id 
        ? 'bg-[#e93949] text-white shadow-md shadow-red-200 dark:shadow-none' 
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-200 font-sans" style={{ fontFamily: "'Inter', sans-serif" }}>
      {/* Toast Notifications */}
      <div className="fixed top-6 right-6 z-50 space-y-3 pointer-events-none">
        {notifications.map(n => (
          <div key={n.id} className={`pointer-events-auto flex items-center gap-3 px-5 py-4 rounded-xl shadow-xl text-white text-sm font-medium animate-in slide-in-from-right duration-300 ${
            n.type === 'error' ? 'bg-red-500' : 'bg-[#e93949]'
          }`}>
      ...
      <div className="flex flex-col lg:flex-row min-h-screen">
        {/* Sidebar */}
        <aside className="w-full lg:w-72 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 p-8 flex flex-col z-10 shadow-sm sticky top-0 h-screen overflow-y-auto">
          {/* Header Section */}
          <div className="mb-12 px-2">
            <img 
              ...
              onError={(e) => { e.target.onerror = null; e.target.style.display='none'; document.getElementById('sidebar-fallback').style.display='flex'; }}
            />
            <div id="sidebar-fallback" className="hidden items-center gap-4">
               <BrandLogo />
               <h1 className="text-xl font-extrabold text-slate-900 dark:text-white">{t('appTitle')}</h1>
            </div>
          </div>
          ...
          <nav className="space-y-2 flex-1">
            ...
            {viewMode === 'public' && (
               <NavItem id="return" icon={RotateCcw} label={t('return')} />
            )}

            <NavItem id="activity" icon={History} label={t('activityLog')} />
            
            {viewMode === 'admin' && (
              <NavItem id="admin" icon={Settings} label={t('adminSettings')} />
            )}
          </nav>
          ...
          <div className="pt-8 border-t border-slate-100 dark:border-slate-700 space-y-6">
             {currentUser ? (
               <div className="flex items-center gap-4 px-2">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#e93949] to-orange-400 flex items-center justify-center text-white font-bold shadow-inner">
                     {currentUser.name.charAt(0)}
                  </div>
                  <div className="flex-1 overflow-hidden">
                     <div className="font-bold text-slate-900 dark:text-white truncate">{currentUser.name}</div>
                     <div className="flex items-center gap-2">
                        <div className="text-xs text-slate-500 font-medium uppercase tracking-wider">{t(currentUser.role)}</div>
                        <button onClick={() => setPasswordModalOpen(true)} className="text-xs text-[#e93949] hover:underline flex items-center gap-1">
                ...
                <div className="px-2 text-sm text-slate-500 font-medium">
                   {t('startTeacher')} Mode
                </div>
             )}
             ...
             <div className="flex gap-3">
                ...
                <button 
                  ...
                  className="flex-1 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 flex justify-center transition-colors"
                  title={t('logout')}
                >
                  <LogOut size={20} />
                </button>
        ...
        {/* Main Content */}
        <main className="flex-1 p-6 lg:p-10 overflow-y-auto min-w-0">
